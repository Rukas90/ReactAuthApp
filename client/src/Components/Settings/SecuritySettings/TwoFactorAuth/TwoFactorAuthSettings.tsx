 ...
 */
const TwoFactorAuthSettings = ({ notifications }: WithNotificationsProps) => {
  ...
  const verify = async (code: string): Promise<boolean> => {
    ...
    try {
      ...
      if (response.success) {
        setActive(true)
        notifications.success(Localized("2FA_VERIFICATION_SUCCESS"))
      ...
      notifications.error(Localized("2FA_CODE_IS_INVALID"))
      return false
    } finally {
      setLoading(false)
    }
...
export default withNotifications(TwoFactorAuthSettings)
