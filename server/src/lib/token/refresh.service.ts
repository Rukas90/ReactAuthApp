    ...
    : Promise<Result<RefreshToken, ResourceMissingError>> =>
{
    ...
    return Result.error(
        new ResourceMissingError("Refresh token is not found.", "REFRESH_TOKEN_NOT_FOUND"))
}
...
export const generateLookupHash = async (token: string): Promise<string> => {
    return await hashing.hmac.hash(token, process.env.REFRESH_TOKEN_LOOKUP_SECRET!)
}
export const validateRefreshToken = async (token: string): Promise<boolean> => {
    ...
    return !refreshToken.revoked && hashing.argon2.compare(token, refreshToken.token_hash)
}